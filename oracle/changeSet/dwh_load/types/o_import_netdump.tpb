CREATE OR REPLACE EDITIONABLE TYPE BODY "O_IMPORT_NETDUMP" AS
  ...
  BEGIN
    ...
    insert /*+ append */into netdump(eth_timestamp, eth_src, eth_dst, ip_src, ip_dst, hostname_src, hostname_dst, ip_len, ip_sum)
    ...
    from ext_netdump en
    log errors into err$_netdump(l_error_code) reject limit unlimited;
    ...
    select count(*) into l_error_rows from err$_netdump
